"""Scene ingest v0 — the buyer-facing "photograph your room" pipeline.

One casual photo -> every furniture item detected, segmented, dimensioned, and
scored. Per-item, per-axis confidence is computed from the two signals the
couch experiment proved out (metrology_experiment.py): distance from camera
and span-direction vs the view axis. Low-confidence items get a directed
capture suggestion ("step right and face the couch end-on") instead of a
silent wrong number.
...
oriented bbox in floor coords -> dims + confidence + category-prior sanity.
  ...
  modal run pipeline/cloud/asset_ingest.py --image "C:\\...\\photo.jpg" --outdir "C:\\...\\out"
"""
...
def ingest(img_bytes: bytes, fpx_exif: float | None = None):
    ...
    with torch.no_grad():
        ...
        pred = dp.infer(dp_transform(np.asarray(pil)), f_px=f_px)
    ...
    # ---------- floor plane: RANSAC over low points, gravity prior = image-down ----------
    step = max(8, W // 400)
    gv, gu = np.mgrid[0:H:step, 0:W:step]
    pts = unproject_grid(gu.ravel(), gv.ravel())
    up_img = np.array([0.0, -1.0, 0.0])  # image up in camera coords (y down in pixels)
    rng = np.random.default_rng(7)
    # candidate floor points: lowest 25% along image-up
    heights = pts @ up_img
    ...
    for _ in range(300):
        ...
        if n @ up_img < 0:
            n = -n
        if n @ up_img < 0.7:  # within ~45 deg of image-up
            continue
        d = n @ s[0]
    ...
    with torch.no_grad():
        gi = gp(images=pil, text=DETECT_PROMPT, return_tensors="pt").to(dev)
        go = gd(**gi)
    det = gp.post_process_grounded_object_detection(
    ...
    for i in order:
        b = boxes[i]
        # drop near-duplicate boxes (IoU > 0.6 with a kept one)
        dup = False
        for j in kept:
            bj = boxes[j]
    ...
    for i in kept:
        box = boxes[i].tolist()
        with torch.no_grad():
            si = sp(pil, input_boxes=[[box]], return_tensors="pt").to(dev)
            so = sam(**si)
        mask = sp.image_processor.post_process_masks(
        ...
        h = (p3 @ n) - d
        # in-plane oriented bbox via PCA
        ...
        height_in = float(np.percentile(h, 98) - max(0.0, np.percentile(h, 2))) * 39.3701
        z_mean = float(p3[:, 2].mean())

        # per-axis confidence: axis direction vs view ray at the object
        view = p3.mean(axis=0)
        view /= np.linalg.norm(view)
        conf, why = [], []
        for k in range(2):
            axis3 = pvt[k, 0] * e1 + pvt[k, 1] * e2
            along = abs(float(axis3 @ view))  # 0 = lateral (good), 1 = receding (bad)
        ...
        label = labels[i]
        prior = PRIORS.get(label.split()[0] if label.split() and label.split()[0] in PRIORS else label)
        prior_flag = ""
...
def main(image: str, outdir: str = ".", fpx: float = 0.0):
    ...
    print(f"camera height {out['camera_height_in']}in | fx {out['fpx']:.0f}")
    for it in out["items"]:
        L, Wd, Hh = it["dims_in"]
